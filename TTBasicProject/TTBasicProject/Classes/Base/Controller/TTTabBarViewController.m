...
- (void)viewDidLoad {
    ...
    self.tabBar.backgroundColor = kCOLOR(244);
...
- (void)_initWithControllers {
    ...
    _fifthVC = [[TTFifthPageViewController alloc] init];
    ...
    self.fifthNav = [[BaseNavigationController alloc] initWithRootViewController:_fifthVC];
    
    self.viewControllers = @[self.firstVC, self.secondVC, self.thirdVC, self.forthVC, self.fifthVC];
}
...
-(void)_initTabBarViewController {
    ...
    NSArray *tabBarImage = @[@"me_normal", @"privatemessage_normal", @"project_normal", @"task_normal", @"tweet_normal"];
    // TabBar上Button在Selected状态下的图片
    NSArray *tabBarPress = @[@"me_selected", @"privatemessage_selected", @"project_selected", @"task_selected", @"tweet_selected"];
    ...
    for (int i = 0; i<[title count]; i++) {
        ...
        if (i == 3) {
            ...
            if (!_msgMarkLab) {
                ...
                _msgMarkLab.layer.cornerRadius = _msgMarkLab.width * 0.5;
